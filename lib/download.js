...
let myconsole = null; // break type-inferring
myconsole = console;
...
function downloadWin(url, dest, opts) {
    return new Promise((resolve, reject) => {
        ...
        myconsole.log('Downloading with Invoke-WebRequest');
...
function download(_url, dest, opts) {
    ...
    return new Promise((resolve, reject) => {
        myconsole.log(`Download options: ${JSON.stringify(opts)}`);
        ...
        https.get(mergedOpts, response => {
            myconsole.log('statusCode: ' + response.statusCode);
            if (response.statusCode === 302) {
                myconsole.log('Following redirect to: ' + response.headers.location);
            ...
            response.on('data', function (chunk) {
                ...
                let n = ~~(progress / (10*1024));
                if (n > 0) {
                    if (myconsole.append) {
                        myconsole.append(".".repeat(n));
                    }
                    progress = 0;
                }
            });
            outFile.on('finish', () => {
                myconsole.log(""); // end of progress
                resolve();
            });
        }).on('error', async err => {
            await fsUnlink(dest);
...
async function getAssetFromGithubApi(opts, assetName, downloadFolder) {
    ...
    if (!opts.force && await fsExists(assetDownloadPath)) {
        myconsole.log('Using cached download: ' + assetDownloadPath);
    ...
    myconsole.log(`Finding release for ${opts.version}`);
    ...
    myconsole.log(`Downloading from ${asset.url}`);
    myconsole.log(`Downloading to ${assetDownloadPath}`);
...
function unzipWindows(zipPath, destinationDir) {
    return new Promise((resolve, reject) => {
        ...
        child_process.exec(expandCmd, (err, _stdout, stderr) => {
            ...
            myconsole.log('Expand-Archive completed');
            resolve();
        });
    });
}
...
function untar(zipPath, destinationDir) {
    return new Promise((resolve, reject) => {
        ...
        unzipProc.on('close', code => {
            myconsole.log(`tar xvf exited with ${code}`);
            if (code !== 0) {
...
module.exports = async opts => {
    ...
    if (opts.quick_try && !await fsExists(assetDownloadPath)) {
        return
    }
    try {
        await getAssetFromGithubApi(opts, assetName, tmpDir)
    } catch (e) {
        myconsole.log('Deleting invalid download cache');
        try {
    ...
    myconsole.log(`Unzipping to ${opts.destDir}`);
    try {
        ...
        myconsole.log('Deleting invalid download');
